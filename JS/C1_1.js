...
const courseInfo = {
    id: "c1_1",
    title: "Strategic Discussions & Decision-Making (C1)",
    focus: "掌握英文策略討論與觀點辯證的表達方式，提升專業溝通能力。"
};
...
answerBtn.addEventListener("click", () => {
    const userResponse = document.querySelector("input[name='c1-test']:checked"); //抓取使用者選擇的答案選項
    const rightAnswer = "C"; //設定正確答案
    ...
    if (userResponse.value === rightAnswer) {
        ...
        if (localStorage.getItem("course_c1_1_rewardClaimed") === "true") {
...
confirmBtn.addEventListener("click", () => {
    localStorage.setItem("course_c1_1_completed", "true"); //紀錄課程已完成狀態
    localStorage.setItem("course_c1_1_rewardClaimed", "true"); //紀錄已領取過獎勵狀態
...
function resetTest() {
    ...
    //清除之前選的答案
    const selectedAnswer = document.querySelector("input[name='c1-test']:checked");
...
function loadCourseStatus() {
    const isCompleted = localStorage.getItem("course_c1_1_completed");
