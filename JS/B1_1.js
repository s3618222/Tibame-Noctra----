...
const courseInfo = {
    id: "b1_1",
    title: "Expressing Opinions at Work (B1)",
    focus: "在職場討論中清楚表達觀點的實用溝通技巧。"
};
...
answerBtn.addEventListener("click", () => {
    ...
    if (userResponse.value === rightAnswer) {
        ...
        if (localStorage.getItem("course_b1_1_rewardClaimed") === "true") {
...
confirmBtn.addEventListener("click", () => {
    localStorage.setItem("course_b1_1_completed", "true"); //紀錄課程已完成狀態
    localStorage.setItem("course_b1_1_rewardClaimed", "true"); //紀錄已領取過獎勵狀態
...
function loadCourseStatus() {
    const isCompleted = localStorage.getItem("course_b1_1_completed");
